...
(function () {
  ...
  function sci(x, digits) {
    ...
    return { c, e, coef: c.toFixed(digits === undefined ? 2 : digits) };
  }
  ...
  function uiFrame() {
    ...
    progressBar.style.transform = 'scaleX(' + clamp(y / docMax, 0, 1).toFixed(4) + ')';
    ...
    for (const sc of scenes) {
      ...
      if (sc.inner) {
        const off = clamp((center - scCenter) * 0.055, -46, 46);
        sc.inner.style.transform = 'translate3d(0,' + off.toFixed(1) + 'px,0)';
      }
    }

    // 滚动提示：一旦开始滚动就淡出
    if (y > 60) scrollHint.classList.add('is-hidden');
  ...
  function renderWishes() {
    ...
    list.forEach((text, i) => {
      ...
      li.addEventListener('mouseenter', () => { engine.highlightWish = i; });
      li.addEventListener('mouseleave', () => { engine.highlightWish = -1; });
