...
(function (global) {
  ...
  const Engine = {
    ...
    resize(initial) {
      ...
      this.reduce = reduce;          // 供主循环使用：降低帧率与时间流速
    ...
    tick(now) {
      ...
      /* 尊重系统「减少动态效果」：此前只是把粒子数砍到 42%，
         整个星域仍在全速流动。现在额外把刷新降到约 30fps、
         并把演化速度放到 0.4 倍 —— 画面依旧在呼吸，但不再"飞"。 */
      if (this.reduce && delta < 31) {
        requestAnimationFrame(this.tickBound);
        return;
      }

      const dt = clamp(delta / 16.667, 0.15, 2.4);
      this.time += delta * 0.001 * (this.reduce ? 0.4 : 1);

      this.update(dt, delta);
      this.draw();

      if (!this.reduce) this.monitorFPS(delta);
      requestAnimationFrame(this.tickBound);
    },
    ...
    update(dt, deltaMs) {
      ...
      for (let i = 0; i < P.length; i++) {
        ...
        if (pActive) {
          ...
          /* 光标揭示（借鉴 canvasui「Particle Reveal」）：
             半径内的粒子被点亮，移开后留下缓慢衰减的余辉 */
          const RR = 150;
          if (d2 < RR * RR) {
            const rf = 1 - Math.sqrt(d2) / RR;
            p.reveal = Math.max(p.reveal || 0, rf * rf);
          }
        }
        ...
        p.reveal = damp(p.reveal || 0, 0.93, dt);        // 光标揭示的余辉衰减
        p.alpha = Math.min(1, (p.px !== null ? tw : tw * 0.86) * p.dim * (1 + p.reveal * 1.15));
    ...
    spawnShooting() {
      if (this.reduce) return;                 // 减少动态效果时不再自动划过流星
